...
<?php require_once('connection.php'); ?>
	...
	if(isset($_POST['send'])  && $_POST['message']!="" ){
		...
		$date = $_POST['date'];
		$sql="INSERT INTO `listeners_msgs`(`Username`, `Date_of_msg`, `Message`) VALUES ('".$_SESSION['MM_Username']."','".date('Y-m-d')."','".$msg."')";
		...
		$msg = mysql_real_escape_string($_POST['message']);
		$date = $_POST['date'];
		$sql="INSERT INTO `feedback`(`Name`, `Email`, `Message`, `DateAdded`) VALUES ('".$_SESSION['MM_Username']."','".mysql_real_escape_string($_POST['email'])."','".mysql_real_escape_string($_POST['message1'])."',now())";
		$run= mysql_query($sql) or die(mysql_error());
