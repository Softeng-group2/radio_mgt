...
.btn2{
	width:120px;
	background:linear-gradient(rgba(255,255,51,1),rgba(0,255,255,1));
	outline:none;
	padding-top:10px;
	padding-bottom:10px;
	padding-left:10px;
...
<ul class="ul">
<li><a href="#inbox"><input type="submit" value="Go to Inbox" style="margin-left:60px;" class="btn"></a></li>
<li><a href="#forum"><input type="submit" value="View Forum" style="margin-left:60px;" class="btn"></a></li>
...
<div id="forum" class="modalDialog">
 ...
 <h2 style="color:rgb(255,255,255);" align="center">Inbox <a href="#help"><input type="image" src="images/help.png" style="margin-left:60px; height:20px; width:20px;"></a></h2>
    ...
    Receipient: <select name="receipient"> <?php
	$adminsql=mysql_query("select * from work_details where Usertype!=''") or die(mysql_error());
	while($row2=mysql_fetch_array($adminsql)){
	 $receipient =$row2['Usertype']; 
	 echo'<option value="'.$row2['Username'].'">'. $receipient.'&nbsp;'.'-'.'&nbsp;'.$row2['Username'].'</option>';
	}
	  ?>
      </select>
      </div>
    ...
    <?php
	$worksql=mysql_query("select * from work_details where Usertype!=''") or die(mysql_error());
	while($row1=mysql_fetch_array($worksql)){
	$receiver =$row1['Usertype'];
	}
    $sql="SELECT * FROM `admin_to_listener` where Sender='".$receiver."' and Receipient='".$_SESSION['MM_Username']."' union SELECT * FROM `admin_to_listener` where Sender='".$_SESSION['MM_Username']."' and Receipient='".$receiver."'  ORDER by `id` asc";
...
while($fetch = mysql_fetch_assoc($run)){
	...
	echo '<br/>'.$fetch['Message'].'<br/>'.'<br/>'.'</div>'.'<div style="background:rgb(255,255,255); padding-top:5px; padding-bottom:5px; margin-bottom:-7px; padding-left:7px;">'.'@'.'&nbsp;'.$fetch['timeofmsg'].'&nbsp;'.'on'.'&nbsp;'.date('l j-M-Y',(strtotime($fetch['dateofmsg']))).'</div>'.'<hr>'.'<br/>';
    ...
    if(isset($_POST['sub']) && !empty($_POST['msgforadmin'])){
		$query = mysql_query("INSERT INTO `admin_to_listener`(`Sender`, `Receipient`, `Message`, `dateofmsg`, `timeofmsg`) VALUES ('".$_SESSION['MM_Username']."','".$receipient."','".$_POST['msgforadmin']."','".date('Y-m-d')."','".date('h:i a')."')") or die(mysql_error());
		}
    if($query){
