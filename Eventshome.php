...
<h1 style="color:rgb(255,255,255); margin-left:200px;">Events Home <a href="adminhome.php"><img src="images/home.jpg" height="40" width="40" style="margin-top:-5px; margin-left:30px;"></a></h1>
<br/><br/><br/><br/>
...
<?php
if(isset($_POST['del'])){
$removequery = mysql_query("DELETE FROM `events` WHERE `Title`='".$_POST['event']."'") or die(mysql_error());	
?>
<script>
...
<iframe src="calender.php"  style="height:700px; float:right; position:absolute; top:0; right:0; width:50%;"></iframe>
 ...
 <h2 style="color:rgb(255,255,255);" align="center">Events For Today</h2>
        ...
        <form action="" method="post" onSubmit="return(confirm('Sure to remove event?'));">
		<table  style="margin-left:200px; margin-top:80px; color:rgb(255,255,255);">
        <tr><th width="150px" colspan="2">Title</th><th>Details</th><th>Event Date</th></tr>
        <?php
		$sql=mysql_query("SELECT * FROM `events` where EventDate = CURRENT_DATE") or die(mysql_error());
		while($get=mysql_fetch_assoc($sql)){
	?>
    <tr><td><input type="radio" name="event" value="<?php echo $get["Title"]; ?>" required class="rbtn" ></td>
    <td><?php echo $get['Title']?></td><td><?php echo $get['Details']?></td><td><?php echo date('l jS F, Y',(strtotime($get['EventDate'])))?></td>
    <td><input type="submit" name="del" value="Remove" class="btndel" /></td>
    </tr>
    <?php
		}
	?>
 ...
 <h2 style="color:rgb(255,255,255);" align="center">All Events</h2>
	...
	echo'<tr>'.'<td>'.$array['Title'].'</td>'.'<td>'.$array['Details'].'</td>'.'<td>'.date('l jS F, Y',(strtotime($array['EventDate']))).'</td>'.'</tr>';	
 ...
 <div id="past" class="modalDialog" style="color:rgb(255,255,255);">
 <h2 style="color:rgb(255,255,255);" align="center">Past Events</h2>
	
		<a href="#close" title="Close" class="close">X</a>
        <table style="margin-left:200px; margin-top:80px; color:rgb(255,255,255);">
        <tr><th>Title</th><th>Details</th><th>Event Date</th></tr>
        <?php
		$sql1=mysql_query("SELECT * FROM `events` where EventDate < CURRENT_DATE") or die(mysql_error());
		while($get1=mysql_fetch_assoc($sql1)){
	?>
    <tr><td><?php echo $get1['Title']?></td><td><?php echo $get1['Details']?></td><td><?php echo date('l jS F, Y',(strtotime($get1['EventDate'])))?></td></tr>
    <?php
		}
	?>
    </table>
 ...
 <h2 style="color:rgb(255,255,255);" align="center">Upcoming Events</h2>
        ...
        <tr><th>Title</th><th>Details</th><th>Event Date</th></tr>
        <?php
		$sql2=mysql_query("SELECT * FROM `events` where EventDate > CURRENT_DATE") or die(mysql_error());
		while($get2=mysql_fetch_assoc($sql2)){
	?>
    <tr><td><?php echo $get2['Title']?></td><td><?php echo $get2['Details']?></td><td><?php echo date('l jS F, Y',(strtotime($get2['EventDate'])))?></td></tr>
    <?php
		}
	?>
    </table>
